...
| [`ncu-microarch`](.github/skills/ncu-microarch/SKILL.md) | 2, 3 | Run `sudo ncu` on a kernel regex; emit ncu_summary.json with SM occupancy, achieved FLOPs, L1/L2 hit, Tensor Core utilization, top warp-stall reason, verdict (compute_bound / memory_bound / latency_bound / low_occupancy / tensor_core_idle). The "why is this kernel slow?" skill. | ✅ v0 implemented (2026-06-09 evening — NCU sudo unlocked by chendi) |
| [`profile-summary-unified`](.github/skills/profile-summary-unified/SKILL.md) | 1, 2, 3 | Merge bench/nsys/torch-profiler/**ncu** outputs into one profile_unified.json with `evidence_chain`. Single source of truth for handoff drafting. | ✅ v0 implemented (ncu adapter added 2026-06-09 evening) |
